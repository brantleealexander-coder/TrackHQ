...
## Phase 4 — Provisioning automation (done — pending end-to-end smoke test)
...
All 10 steps now have real implementations + mocked-HTTP tests (58 tests passing). The "under 60 min" criterion still needs a real end-to-end run to validate; that requires pushing the TrackHQ repo to GitHub first (currently local-only).
...
- **4d (done):** VAPI — `vapi_create` + `vapi_webhook_update`. `lib/clients/vapi.ts` covers create/list/PATCH; `lib/vapi/render.ts` is a TS port of `template-server/app/render_vapi.py`. `lib/vapi/business-config.ts` builds the substitution map from the manifest's optional `receptionist:` block (sensible defaults: Alex / 11labs Rachel / America/Chicago, transfer phone from `contact.phone`). `vapi_create` is idempotent (lists assistants and reuses-by-name if a previous run created one without persisting the id). Original `vapi_patch` step dropped: POST accepts the full body, so create + PATCH was redundant. Step count: 14 → 13.
- **4e (done):** Vercel — `vercel_create` (single step, was 3). `lib/clients/vercel.ts` wraps get / create / upsert-env. Step builds the env vars list (NEXT_PUBLIC_TENANT_CONFIG_JSON from manifest, Supabase URLs/keys from prior state, Mapbox from .env) and posts them as part of the project create — Vercel auto-deploys from the linked GitHub fork. Idempotent: if the project already exists, env vars are upserted instead of failing. The original `vercel_envs` + `vercel_deploy` stubs are dropped (Vercel's API supports envs-in-create and the *.vercel.app domain is predictable from the project name). Step count: 13 → 11.
- **4f (done):** Railway — `railway_create` + `railway_deploy`. `lib/clients/railway.ts` is a minimal GraphQL wrapper over `backboard.railway.app/graphql/v2` (project create, service-from-repo create, serviceInstance update for rootDirectory, bulk variable upsert, latest-deployment query, serviceDomainCreate). `railway_create` persists `project_id` + `environment_id` *before* the service-create call so a mid-step crash resumes against the same project (same orphan-prevention pattern as `supabase_create`). `railway_deploy` polls until SUCCESS, then generates the public *.up.railway.app domain (falls back to `deployment.staticUrl` if the domain mutation reports "already exists"). 10-step pipeline finalized.
- **4g (done):** runbook generator — `lib/runbook.ts` + `lib/steps/runbook.ts`. Writes `provisioning/state/<slug>.runbook.md` with a resource table (GitHub fork URL, Supabase ref, Vercel + Railway URLs, VAPI assistant id) and a conditional checklist: enable Actions on the fork, drop in `business_config.json`, link a phone number to VAPI, DNS (if `provisioning.domain` set), Google Calendar OAuth (if `features.google_calendar`), Twilio SMS env vars (if `features.twilio_sms`), Telegram bot (if `features.telegram_alerts`), and a final smoke-test checklist.
